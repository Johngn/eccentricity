...
dt = 0.02
r = 1
sigma = 17000*r**(-1/2)*au**2/solarmass
lim = 1.5
...
def acceleration(W0):
    ...
    # vradius = np.sqrt(V[0]**2+V[1]**2)
    # vtheta = np.arctan2(V[1],V[0])
    # vz = V[2]
    
    # Rpolar = np.array([radius, theta, z])
    # Vpolar = np.array([vradius, vtheta, vz])
    
    theta = np.arctan2(R[1],R[0])
    
    u_r = np.array([np.cos(theta), np.sin(theta), 0])                           # radial unit vector
    # u_a = -np.sin(theta)*np.array([1,0,0])+np.cos(theta)*np.array([0,1,0])    # azimuthal unit vector
    u_a = np.array([-np.sin(theta), np.cos(theta), 0])
    u_z = np.array([0,0,1])

    vr = np.dot(V,u_r)                     # radial velocity
    va = np.dot(V,u_a)                   # tangential velocity
    vz = np.dot(V,u_z)
    # print(vz)
    # print(u_a)
    # print(thetahat)
    ...
    # CN prescription
    # tau_e = t_wave/0.78*(1-0.14*ehat**2+0.06*ehat**3+0.18*ehat*ihat**2)
    # tau_i = t_wave/0.544*(1-0.30*ihat**2+0.24*ihat**3+0.14*ihat*ehat**2)
    # Pe = (1+(ehat/2.25)**1.2+(ehat/2.84)**6)/(1-(ehat/2.02)**4)
    # tau_a = t_wave*2/h**2/(2.7+1.1*0.5)*(Pe+Pe/np.abs(Pe)*(0.07*ihat+0.085*ihat**4-0.08*ehat*ihat**2))
    
    dvdt2 = -vk/2/tau_a*u_a - vr/tau_e*u_r - (va-vk)/tau_e*u_a - vz/tau_i*u_z  # equation 46 in Ida 2020
    # dvdt2 = -V/tau_a -2*np.dot(V,R)*R/r**2/tau_e -vz/tau_i*zhat # equation 15 in Creswell+Nelson 2008
...
H = np.cross(R,V)
i_total = np.arccos(H[:,2]/np.linalg.norm(H, axis=1))
...
# e_results_low = np.zeros(noutputs)
# e_results_mid = np.zeros(noutputs)
i_results_high = np.zeros(noutputs)
...
for j in range(noutputs):    
    ...
    # tau_e = t_wave/0.78*(1-0.14*ehat**2+0.06*ehat**3+0.18*ehat*ihat**2)   # equation 11 from Creswell+Nelson 08
...
ax.plot(np.arange(0,noutputs,1)*dt/2/np.pi*omegak, i_results_high, c='tab:blue')
...
ax.plot(W[:,1], W[:,2], linewidth=0.01, c='steelblue')
